...
export default defineEventHandler(async (event) => {
  ...
  const result = await prisma.contaPagar.updateMany({ where: { id, empresa_id: companyId, status: "pendente" }, data: { status: "cancelado" } });
  if (!result.count) throw createError({ statusCode: 409, message: "A conta não existe ou não está pendente." });
  const data = await prisma.contaPagar.findUniqueOrThrow({ where: { id } });
  await logger.update(event, JSON.stringify({ modulo: "financeiro", tipo: "cancelamento_conta_pagar", id }));
