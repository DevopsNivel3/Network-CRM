...
// Rota para deletar um comentário na oportunidade
export default defineEventHandler(async (event) => {
  try {
    ...
    const anexos = await prisma.oportunidadeComentarioAnexo.findMany({
      where: { comentario_id: body.comment_id },
      select: { url: true },
    });

    const data = await prisma.oportunidadeComentarios.delete({
      where,
    });
