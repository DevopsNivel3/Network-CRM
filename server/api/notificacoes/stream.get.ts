...
import { subscribeNotificationStream } from "@/server/utils/notificationStream";
...
export default defineEventHandler(async (event) => {
  try {
    ...
    const unsubscribe = subscribeNotificationStream(usuario.id, (payload) => {
      send(payload);
    });
    ...
    throw createError({
      statusCode: 400,
      message: err?.message || "Erro ao abrir stream de notificações",
    });
  }
});
